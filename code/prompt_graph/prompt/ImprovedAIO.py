...
class ImprovedAIO(LightPrompt):
    def __init__(self, token_dim, token_num, cross_prune=0.1, inner_prune=0.6):
        super(ImprovedAIO, self).__init__(token_dim, token_num, 1, inner_prune)  # only has one prompt graph.
    ...
    def forward(self, graph_batch: Batch):
        ...
        for g in Batch.to_data_list(graph_batch):
            ...
            # cross_sim = torch.where(cross_sim < torch.topk(cross_sim, min(g.x.shape[0], max(int(average_degree), 2)), dim=1)[0][:, -1].unsqueeze(1), 0, cross_sim)
...
class FrontAndHead(torch.nn.Module):
    def __init__(self, input_dim, hid_dim=16, num_classes=2,
        ...
        self.PG = ImprovedAIO(token_dim=input_dim, token_num=token_num, cross_prune=cross_prune,
